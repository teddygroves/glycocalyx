...
We constructed five models (see more details in Supplementary Note: Statistical models): 
    ...
    Model of relative glycocalyx fluorescence intensity (glycocalyx maps; Fig. 3) from the data obtained with dual-lectin labeling. We used a linear regression on log scale, with random intercepts per vessel type and per lectin/vessel type interaction class. 
